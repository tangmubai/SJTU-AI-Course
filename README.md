...
- 课程讨论：首页和模拟测试页各自嵌入独立的 GitHub Discussions；也可浏览全部主题或在 GitHub 中发起新讨论。

## GitHub Discussions 嵌入

讨论区通过 [giscus](https://giscus.app/zh-CN) 连接仓库的 GitHub Discussions。网站按页面路径创建并查找讨论，因此首页与模拟测试页各自有独立帖子；访客首次评论或回应时，giscus 会自动创建对应帖子。页面同时提供“浏览全部讨论”和“发起新讨论”入口，后者使用开放的 `General` 分类，支持任意数量的课程主题。
